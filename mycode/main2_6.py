import argparse
import torch
import dataloader_deap
from models import main_models_6
import numpy as np
import os
# python main1.py 5 120 50 's05' 's01' 112 64
from sklearn import metrics
parser=argparse.ArgumentParser()
parser.add_argument('n_epoches_1',type=int,default=5)
...
parser.add_argument('n_target_samples', type=int, default=180 ) ###
...
o_data = opt['original']
bb= opt['target']
train_dataloader = dataloader_deap.source_loader(o_data)
# train_dataloader = dataloader.mnist_dataloader(batch_size = opt['batch_size'], train=True, )
test_dataloader = dataloader_deap.source_loader(bb)

classifier = main_models_6.Classifier()
encoder = main_models_6.Encoder()
discriminator = main_models_6.DCD(input_features=128)
...
if __name__=='__main__':
    ...
    X_s, Y_s = dataloader_deap.sample_data(o_data)  ##
    # print("X_s.shape", X_s.shape)
    X_t, Y_t = dataloader_deap.create_target_samples(opt['n_target_samples'], bb)  ## 112
    ...
    for epoch in range(opt['n_epoches_2']):
        # data
        groups, aa = dataloader_deap.sample_groups(X_s, Y_s, X_t, Y_t, seed=epoch)  ##  组成样本
    ...
    test_dataloader = dataloader_deap.target_loader(bb)
    accuracy_all2 = []
    auc_all = []
    ...
    if isExists:
        ...
        os.makedirs(path)
    path2 = './/deap_6//' + str(bb) + "//save_model"
    ...
    for epoch in range(opt['n_epoches_3']):
        ...
        groups, groups_y = dataloader_deap.sample_groups(X_s, Y_s, X_t, Y_t, seed=opt['n_epoches_2'] + epoch)
